  ...
  for (i = 1; i <= atom->ntypes; i++) {
    for (j = i; j <= atom->ntypes; j++) {
      ...
      MPI_Bcast(&setflag[i][j],1,MPI_INT,0,world);
			if (setflag[i][j]) {
				if (me == 0) {
          utils::sfread(&kn[i][j],sizeof(double),1,fp,NULL,error);
					utils::sfread(&kt[i][j],sizeof(double),1,fp,NULL,error);
					utils::sfread(&gamman[i][j],sizeof(double),1,fp,NULL,error);
					utils::sfread(&gammat[i][j],sizeof(double),1,fp,NULL,error);
					utils::sfread(&xmu[i][j],sizeof(double),1,fp,NULL,error);
					utils::sfread(&dampflag[i][j],sizeof(int),1,fp,NULL,error);
					utils::sfread(&cut[i][j],sizeof(double),1,fp,NULL,error);
				}
...
{
	if (comm->me == 0) {
    utils::sfread(&cut_global,sizeof(double),1,fp,NULL,error);
	}
