...
void PairGranHookeHistoryStoch::settings(int narg, char **arg)
{
  ...
  xmu_min = utils::numeric(FLERR, arg[4], false, lmp); // min. bound on friction
  xmu_max = utils::numeric(FLERR, arg[5], false, lmp); // max. bound on friction
  kdist_a = utils::numeric(FLERR, arg[6], false, lmp); // parameter a of the beta-Kumaraswamy distribution
  kdist_b = utils::numeric(FLERR, arg[7], false, lmp); // parameter b of the beta-Kumaraswamy distribution
  seed    = utils::numeric(FLERR, arg[8], false, lmp); // seed
  ...
  limit_damping = 0;
  if (narg == 11) {
    if (strcmp(arg[10], "limit_damping") == 0)
      limit_damping = 1;
    else
