...
#ifdef PAIR_CLASS
// clang-format off
PairStyle(gran/hooke/history/stoch,PairGranHookeHistoryStoch);
// clang-format on
#else

#ifndef LMP_PAIR_GRAN_HOOKE_HISTORY_STOCH_H
#define LMP_PAIR_GRAN_HOOKE_HISTORY_STOCH_H

#include "pair.h"

namespace LAMMPS_NS {

class PairGranHookeHistoryStoch : public Pair {
 public:
  PairGranHookeHistoryStoch(class LAMMPS *);
  virtual ~PairGranHookeHistoryStoch();
 ...
 protected:
  ...
  double xmu, xmu_min, xmu_max, kdist_a, kdist_b;
  int distribution;
  int dampflag;
  double dt;
  ...
  int seed;
  double get_stoch_friction_coeff(double, double, double, double);
};
